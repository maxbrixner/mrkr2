export class FilteredTable extends HTMLElement {
    ...
    updateContent() {
        ...
        this._queryContent().then(content => {
            ...
            document.querySelector('message-box')?.showMessage(`Unable to load table content.`, 'error', error.message);
            this._table.classList.remove('loading');
        });
    }
    _addHeaders(content) {
        ...
        for (const item of content) {
            ...
            for (const key in item) {
                ...
                if (display === 'text') {
                    ...
                    divElement.style.backgroundColor = this._configParsed?.chips?.[item[key]] || '#e0e0e0';
    ...
    _onSelectionChangeEvent(rowId) {
        return (event) => {
            const checkboxes = Array.from(this._table.querySelectorAll('input[type="checkbox"][name="select-row"]'));
